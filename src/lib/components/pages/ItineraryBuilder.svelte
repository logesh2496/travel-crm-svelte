<script lang="ts">
  ...
  async function downloadPDF() {
    ...
    try {
      ...
      const opt = {
        margin:       10,
        filename:     `${itinTitle || 'Itinerary'}.pdf`,
      ...
      html2pdf().set(opt).from(element).save().then(() => {
        onAction('toast', { msg: 'PDF Downloaded successfully!', type: 'success' });
      });
    } catch (e) {
      console.error(e);
  ...
  async function emailPDF() {
    onAction('toast', { msg: 'Preparing email attachment...', type: 'info' });
    const element = document.getElementById('pdf-export-content');
    if (!element) return;
    ...
    try {
      ...
      onNavigate('communication', { 
        ...
        selectedItineraryId: currentItineraryId || '',
        pdfBase64: pdfBase64,
        pdfName: `${itinTitle || 'Itinerary'}.pdf`
      });
      
    } catch (e) {
      console.error(e);
      onAction('toast', { msg: 'Failed to prepare PDF', type: 'error' });
    }
  }
</script>
      ...
      <div class="card" style="display:flex; flex-direction:column; justify-content:center;">
        <div style="background:var(--bg2); border-radius:var(--radius-lg); padding:24px; text-align:center; border: 1px solid var(--border);">
          ...
          <div style="display: flex; gap: 8px; margin-top: 24px;">
            <button class="btn btn-teal btn-sm" style="flex: 1;" onclick={saveItinerary} type="button"><i class="ti ti-device-floppy"></i>Save Itinerary</button>
            <button class="btn btn-primary btn-sm" style="flex: 1;" onclick={() => activeTab = 'builder'} type="button"><i class="ti ti-arrow-left"></i>Go to Build Itinerary</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div style="margin-top: 32px; display:flex; justify-content:center;">
      <div style="width: 100%; max-width: 800px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 16px;">
          ...
          <div style="display:flex; gap:8px;">
            <button class="btn btn-primary btn-sm" onclick={downloadPDF} type="button"><i class="ti ti-download"></i>Download PDF</button>
            ...
            <button class="btn btn-sm" onclick={emailPDF} type="button"><i class="ti ti-mail"></i>Email PDF</button>
          </div>
        </div>
